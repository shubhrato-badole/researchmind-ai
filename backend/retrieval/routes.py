...
def search(
    ...
    # Check daily document search limit
    allowed, remaining, limit = check_daily_limit(
        user_id,
        "doc_search"
    )
    ...
    if not allowed:
        raise HTTPException(
            status_code=429,
            detail=f"Daily doc search limit reached ({limit}/day). Upgrade to Pro for unlimited."
        )
